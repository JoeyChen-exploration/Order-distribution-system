...
async function readFile(path) {
  ...
  ws.eachRow({ includeEmpty: true }, (row) => {
    ...
    for (let c = 1; c <= colCount; c++) {
      rowData.push(normalizeCell(row.getCell(c).value))
    }
    rows.push(rowData)
  })
...
async function main() {
  const manual = await readFile("06(1).xlsx")
  const algo = await readFile("排单结果_20260311.xlsx")
  ...
  manual.data.forEach((r) => {
    if (r["订单号"]) manualMap[r["订单号"]] = r
  })
  const algoMap = {}
  algo.data.forEach((r) => {
    if (r["订单号"]) algoMap[r["订单号"]] = r
  })
