...
服务端运行时与初始化脚本统一通过 `DATABASE_URL` 读取数据库路径，默认值为：

```bash
file:./prisma/dev.db
```

涉及文件：`lib/database-url.ts`、`lib/db.ts`、`prisma/init-db.ts`、`prisma/seed.ts`。

### 2) 派单确认原子化

新增后端接口 `POST /api/orders/assign`，将“司机状态更新 + 订单绑定司机 + 改派释放原司机”放入同一事务，避免前端分步请求导致的半成功状态。

- 订单详情页「派单/改派」与排单控制台「确认派单/确认全部派单」均改为调用该接口。
- 接口会校验：订单是否可派、司机是否在岗（available/busy）。

### 3) 导入格式统一为 XLSX

- 订单导入页面与导入弹窗均改为仅接受 `.xlsx`。
- 导入模板下载改为 `.xlsx`（不再下载 csv 模板）。
...
- 后端批量派单接口（`/api/orders/assign-batch`）也做同样校验，防止绕过前端。
...
| 导入仅支持 xlsx | 导入入口和模板下载已统一为 xlsx，避免格式不一致 |
