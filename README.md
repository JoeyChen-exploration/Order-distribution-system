...
| 司机车型 | 可接订单 | 备注 |
|---|---|---|
| 豪华商务型 | 豪华商务型、普通商务型、豪华型、舒适型 | 不接经济型 |
| 普通商务型 | 普通商务型、豪华型、舒适型 | 不接经济型 |
| 豪华型 | 豪华型、舒适型 | 不接经济型，接舒适型属降级 |
| 舒适型 | 舒适型、经济型 | |
| 经济型 | 经济型 | |

> **商务型订单细分**：从 Excel 导入的「商务型」订单需在订单管理中手动细分为「豪华商务型」或「普通商务型」（存储在 `metadata["商务类型"]`）。未细分的订单无法参与智能/批量派单，订单管理页顶部会显示黄色警告条列出待处理订单号。
...
```
① 按服务时间升序排列待派订单（最早服务时间优先）
↓
② 并发预取所有「司机有效位置 → 订单上车点」路线
   （22 司机 × ~26 唯一上车点 ≈ 572 条，去重后并发请求）
   进度：预取路线数据 X / 572
↓
③ 对每条订单运行单条派单算法（使用预取的 TravelTimeCache）
↓
④ 派出后将该订单写入本地列表（status=1）
↓
⑤ 后续订单的冲突检测感知到同批次已分配的订单
```
...
**商务类型**存储在 `order.metadata["商务类型"]`（值为 `"豪华商务型"` 或 `"普通商务型"`），同样在订单管理下拉菜单中设置。

订单管理页顶部警告条：
- **橙色**：包车订单未选时长（影响 serviceBuffer 计算）
- **黄色**：商务型订单未细分类型（无法参与任何派单）

两类待确认订单均会**阻断批量派单**，必须处理后才能继续。
...
|---|---|---|
| 订单号 | ✓ | 唯一标识 |
| 预订车型 | ✓ | 舒适型/豪华型/商务型/经济型（商务型需导入后手动细分） |
...
| 商务型需手动细分 | Excel 只有"商务型"一列，需在订单管理中逐条选择豪华商务/普通商务 |
